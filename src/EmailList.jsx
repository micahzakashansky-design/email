          ...
          {emails.length === 0 ? (
            ...
            emails.map((email) => (
              <motion.div
                ...
                className={`p-4 border-b border-white/10 cursor-pointer hover:bg-white/5 transition-all group relative focus-visible:ring-2 focus-visible:ring-blue-500 outline-none ${
              ...
              >
                ...
                <div className="flex items-center space-x-3" onClick={(e) => e.stopPropagation()}>
                  <button
                    ...
                    className={`${email.isImportant ? 'text-yellow-500 opacity-100' : 'text-gray-400 hover:text-yellow-500 opacity-0 group-hover:opacity-100 group-focus-within:opacity-100'} transition-all duration-200`}
                  ...
                  <div className="flex items-center space-x-3">
                    {email.status === 'inbox' && (
                      <div className="flex items-center space-x-3 opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition-opacity">
                        <button
                    ...
                    {(email.status === 'later' || email.status === 'done') && (
                      <div className="opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition-opacity flex items-center space-x-3">
                        {email.status === 'later' && (
                          <button
                            onClick={() => onMarkDone(email.id)}
                            className="text-gray-400 hover:text-green-500 transition-colors"
